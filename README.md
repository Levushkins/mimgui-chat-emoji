# Смайлы из `_chat.asi` в mimgui и диалогах SA-MP

Разбор плагина чата Arizona Games (`_chat.asi`), извлечение всех **1584 смайлов и иконок**
в том же порядке, что и в панели чата, и готовый модуль, который умеет две вещи:

* **рисовать их в mimgui** - своей текстурой, поэтому работает на любом сервере;
* **вставлять их в обычные диалоги SA-MP** - там рисует уже сам плагин чата,
  от скрипта нужна только строка с токеном. Ни текстуры, ни mimgui для этого
  не требуется.
...
* **на сервере без плагина** токен останется видимым мусором `:u1f603:`,
...
Примеров три, кладите **что-то одно**:

| файл | команда | что показывает |
|------|---------|----------------|
| `chat_emoji_minimal.lua` | `/sm`   | четыре способа применения в двадцати строках |
...
MoonLoader запускает все `.lua` из своей папки, так что два примера сразу
просто дважды дёрнут один и тот же атлас. Текстура при этом всё равно
останется одна на процесс (см. «Несколько скриптов сразу»), но смысла в этом
немного.
...
```lua
emoji.dialog(1235, 'Игроки',
    'Ник\tУровень\tДеньги\n' ..
    emoji.tok('crown') .. ' Nick_Name\t12\t1.250.000 ' .. emoji.tok('cash'),
    'Открыть', 'Назад', 5)
```

`emoji.dialog` - тонкая обёртка над `sampShowDialog` с тем же порядком
аргументов. Она делает ровно две вещи:

1. прогоняет заголовок и текст через `emoji.fit()`;
2. не отправляет текст длиннее `emoji.DIALOG_LIMIT` (по умолчанию 4096 байт),
   а возвращает `false, причина`. Слишком длинный текст диалога обрезается
   молча, и ловить это в бою неприятно.

Обёртка не обязательна - `sampShowDialog(id, caption, text, b1, b2, style)`
с токенами в строке работает и напрямую.

#### Если сервер не Arizona

Токен - обычный текст, и там, где его некому отрисовать, он таким и
останется. `emoji.fit()` решает это сам:

```lua
emoji.fit('привет :u1f603: как дела')
-- на Arizona:      привет :u1f603: как дела
-- на другом:       привет как дела
```

Проверка идёт по загруженному в процесс модулю плагина
(`emoji.detect()`), результат запоминается. Если проверка по имени модуля
вам не подходит, выставьте `emoji.plugin = true` или `false` руками.

#### Про кодировку в диалогах

`sampShowDialog` ждёт cp1251, как и весь SA-MP. Токен состоит из цифр и
латиницы, поэтому перекодировка его не трогает - переводить надо только
свой русский текст, ровно как обычно:

```lua
emoji.dialog(id, u8:decode(заголовокИзImGui), u8:decode(текстИзImGui), ...)
```

#### Посмотреть вживую

`chat_emoji_dialogs.lua`, команда `/emjd`: выбираете стиль диалога,
набираете текст, кликом вставляете смайлы из панели и сразу открываете
настоящий диалог. Ответ на него показывается в том же окне, а сверху видно,
нашёлся ли плагин чата. Есть четыре готовых примера - меню, таблица,
сообщение и поле ввода.
...
| функция | описание |
|---------|----------|
| `emoji.loadTable([dir])` | только таблицы: имена, коды, категории. Без текстуры, без mimgui - для чата и диалогов |
| `emoji.load([dir])` | то же плюс текстура, из `imgui.OnInitialize`; `true` либо `false, ошибка` |
| `emoji.unload()` | освободить текстуру (в `onScriptTerminate`) |
| `emoji.ready` | таблицы построены |
| `emoji.loaded` | таблицы плюс текстура: можно рисовать |
...
| функция | описание |
|---------|----------|
| `emoji.dialog(id, caption, text, b1, b2, style)` | `sampShowDialog` с `fit()` и проверкой длины; `true` либо `false, причина` |
| `emoji.detect()` | загружен ли плагин чата, то есть отрисуются ли токены |
| `emoji.fit(text)` | текст как есть при наличии плагина, без токенов при его отсутствии |
| `emoji.strip(text)` | убрать токены вместе с лишним пробелом |
| `emoji.expand(text)` | `:trophy:` → `:u1f3c6:`; заменяет только имена из атласа |
| `emoji.measure(text)` | `{ bytes, tokens, tokenBytes, visible }` |
| `emoji.plugin` | результат `detect()`; можно выставить руками |
| `emoji.DIALOG_LIMIT` | порог длины текста диалога, по умолчанию 4096 |
...
{ name = 'smiley', cp = 0x1F603, cat = 'Смайлы',
  slot = 20, index = 21, token = ':u1f603:', cells = 1,
  u0 = 0.39, v0 = 0.0, u1 = 0.41, v1 = 0.02,     -- числа, есть всегда
  uv0 = ImVec2(...), uv1 = ImVec2(...) }         -- только если есть mimgui
```

`uv0`/`uv1` готовы для `imgui.Image` и `DrawList:AddImage`, а `u0…v1` те же
координаты числами - они есть даже когда mimgui в скрипт не подключён.
